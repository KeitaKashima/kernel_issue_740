...
void Processing_Before_Start_Kernel(void)
{
     ...
     */
#ifndef TOMO
	if (pdPASS != xTaskCreate(main_task, "MAIN_TASK", 768, NULL, 3, &main_task_handle))
#else
	if (pdPASS != xTaskCreate(main_task, "MAIN_TASK", 768, NULL, configMAX_PRIORITIES - 1, &main_task_handle))
#endif
    {
        while(1)
        {
