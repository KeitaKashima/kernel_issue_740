...
static void __near r_Config_TAU0_1_interrupt(void)
{
    ...
    /* Get task handle */
    tskhdl_interrupt = xTaskGetCurrentTaskHandle();

    /* g_xHigherPriorityTaskWoken must be initialized to pdFALSE. */
    g_xHigherPriorityTaskWoken = pdFALSE;

    /* Set event "INTERRUPT_PERIODIC_TIMER" in event_group_handle. */
    xResult = xEventGroupSetBitsFromISR (event_group_handle,
    ...
    if (xResult != pdFAIL)
    {
        /* If g_xHigherPriorityTaskWoken is now set to pdTRUE then a context
         switch should be requested. The macro used is port specific and will
         be either portYIELD_FROM_ISR() or portEND_SWITCHING_ISR() - refer to
         the documentation page for the port being used. */
        portYIELD_FROM_ISR(g_xHigherPriorityTaskWoken);
    }
    /* End user code. Do not edit comment generated here */
}
